...
namespace MicroserviceProject.Discount.Api.Features.Discounts
{
    public static class DiscountEndpointExt
    {
        ...
        {
            app.MapGroup("api/v:{version:apiversion}/discounts").WithTags("Discounts").WithApiVersionSet(apiVersionSet).CreateDiscountGroupItemEndpoint().GetDiscountByCodeGroupItemEndpoint().RequireAuthorization("Password");
        }
    }
}
